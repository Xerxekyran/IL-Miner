...
import de.digitalforger.epqLearner.util.fileConverter.impl.GoogleClusterRowReader;
 ...
 *
 */
public class GoogleClusterFileEventSource extends FileEventSource {
	private static Logger logger = Logger.getLogger(GoogleClusterFileEventSource.class.getName());
	 ...
	 */
	public GoogleClusterFileEventSource(List<String> filesToRead, LinkedList<Long[]> foundMatches, long deltaT) throws FileNotFoundException {
		super(filesToRead.get(0), new GoogleClusterRowReader(), new TraceChunkerFromMatches(foundMatches), deltaT);
