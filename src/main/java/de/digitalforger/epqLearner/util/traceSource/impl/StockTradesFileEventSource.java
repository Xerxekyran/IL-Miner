 ...
 *
 */
public class StockTradesFileEventSource extends FileEventSource {

	private static Logger logger = Logger.getLogger(StockTradesFileEventSource.class.getName());
	 ...
	 */
	public StockTradesFileEventSource(List<String> filesToRead, LinkedList<Long[]> foundMatches, long deltaT) throws FileNotFoundException {
		super(filesToRead.get(0), new StockTradesRowReader(), new TraceChunkerFromMatches(foundMatches), deltaT);
	...
	protected void continueFileReading() {
		...
		try {
			while (null != (row = fileReader.readLine())) {								
				...
				// the event may be null for the headline row
				if (curEvent == null) {
					continue;
				}
